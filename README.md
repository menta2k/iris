...
| **Inbound mail webhooks** | Forward received mail to an HTTP endpoint: register `(recipient, URL)` pairs (an exact address like `support@kmx.example.com` or a whole-domain catch-all) and KumoMTA POSTs the raw message (`Content-Type: message/rfc822`, with `X-Iris-Recipient` / `X-Iris-Message-Id` headers) to your endpoint. Optional per-route secret adds an `X-Iris-Signature` (HMAC-SHA256); KumoMTA's queue handles retries. Manage at `/policy/mail-webhooks`; applies on Policy → Apply (the recipient's domain MX must point at this KumoMTA). |
| **Inbound spam filtering (rspamd)** | Scan mail received for your hosted domains through [rspamd](https://rspamd.com)'s HTTP `/checkv2` and honor its verdict: `reject` → SMTP 550, `soft reject`/`greylist` → 451 defer, `add header`/`rewrite subject` → accept + `X-Spam-*` headers. Two modes — **Tag only** (scan + headers, never reject; monitor mode) and **Enforce** (act on the verdict). Scoped to inbound-to-hosted recipients only (outbound relay and API-submitted mail are never scanned); **fail-open** if rspamd is unreachable so a scanner hiccup never blocks real mail. Configure mode + URL in **Global Settings**; applies on Policy → Apply. |
...
| `IRIS_WEBAUTHN_RP_ID` | *(unset → passkeys disabled)* | WebAuthn Relying Party ID — the registrable domain (e.g. `kmx.example.com`). |
| `IRIS_WEBAUTHN_RP_ORIGINS` | *(unset)* | Comma-separated full origins users hit (e.g. `https://kmx.example.com:8443`). Must match the browser's address bar exactly. |
