...
Outbound deliverability hygiene (set in **Global Settings → Other**, not env): an **Outbound EHLO hostname** field renders a default egress `ehlo_domain` so KumoMTA stops announcing the bare system hostname (`rspamd HFILTER_HELO_5`); a per-VMTA HELO name still overrides it. The renderer also auto-adds `Date:` and `Message-ID:` to submitted/injected mail that lacks them (`rspamd MISSING_DATE` / `MISSING_MID`), before DKIM signing. Both take effect on the next **Apply**.
