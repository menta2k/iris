...
func newService(t *testing.T) *service.Service {
	...
	return service.NewService(service.Deps{
		...
		KumoConfig: biz.NewKumoConfigUsecase(
			data.NewKumoConfigRepo(outboundRepo, domainSafetyRepo, data.NewInboundRepo(db), data.NewInboundRouteRepo(db), data.NewFBLRepo(db), data.NewWarmupRepo(db), data.NewBlueprintRepo(db)), biz.NewStubKumoMTA(), mailOpsRepo, auditor,
			settingsUC),
		Settings:   settingsUC,
