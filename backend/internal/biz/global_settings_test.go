...
func TestGlobalSettingsValidate(t *testing.T) {
	...
	assertReason(t, (&GlobalSettings{DMARCReportEmail: "dmarc@kmx.example.com"}).Validate(), "")
	assertReason(t, (&GlobalSettings{DMARCReportEmail: ""}).Validate(), "")
	assertReason(t, (&GlobalSettings{DMARCReportEmail: "notanemail"}).Validate(), "SETTINGS_DMARC_EMAIL_INVALID")
...
func TestAdminSettingsValidation(t *testing.T) {
	...
	s = &GlobalSettings{AdminHTTPAddr: ":8080", AdminTLSEnabled: true, AdminTLSCertDomain: "*.kmx.example.com", AcmeRenewInterval: "12h", AcmeRenewBefore: "30d"}
	if err := s.Validate(); err != nil {
