...
func TestKumoLogFromHeader(t *testing.T) {
	...
	rec, err := ParseKumoLogRecord([]byte(`{
		"type":"Delivery","id":"m1",
		"sender":"b+abc.def@bounce.kumo.verax.net","recipient":"x@dest.example",
		"headers":{"From":"Monitoring <sentry@infra.verax.net>","Subject":"hi"}}`))
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	if got := rec.FromHeader(); got != "Monitoring <sentry@infra.verax.net>" {
