...
func TestListQueuesDelegatesToAdmin(t *testing.T) {
	q := &fakeQueueAdmin{summary: []*QueueState{{Domain: "example.com", Depth: 7}}}
	got, err := queueUC(q).ListQueues(ownerCtx())
	if err != nil || len(got) != 1 || got[0].Domain != "example.com" || got[0].Depth != 7 {
...
func TestQueueActionSuspendResume(t *testing.T) {
	q := &fakeQueueAdmin{}
	uc := queueUC(q)
	if _, err := uc.RequestQueueAction(ownerCtx(), "suspend", "example.com", "maint", ""); err != nil {
		t.Fatalf("suspend: %v", err)
	}
	if q.lastAction != "suspend" || q.lastDomain != "example.com" {
		t.Fatalf("suspend not dispatched: %+v", q)
	}
	if _, err := uc.RequestQueueAction(ownerCtx(), "resume", "example.com", "", ""); err != nil {
...
func TestQueueBounceRequiresConfirmation(t *testing.T) {
	...
	if _, err := uc.RequestQueueAction(ownerCtx(), "bounce", "example.com", "", ""); err == nil {
	...
	if _, err := uc.RequestQueueAction(ownerCtx(), "bounce", "example.com", "", "c1"); err != nil {
...
func TestQueueActionInvalidVerb(t *testing.T) {
	if _, err := queueUC(&fakeQueueAdmin{}).RequestQueueAction(ownerCtx(), "drain", "example.com", "", ""); err == nil {
