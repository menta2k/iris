...
func TestDomainCheckAllPass(t *testing.T) {
	loader := fakeLoader{snap: ConfigSnapshot{
		...
		DKIM:      []*DKIMDomain{{Domain: "kmx.jobs.bg", Selector: "default"}},
	}}
	dns := fakeResolver{
		mx:   map[string][]*net.MX{"kmx.jobs.bg": {{Host: "mx.kmx.jobs.bg.", Pref: 10}}},
		host: map[string][]string{"mx.kmx.jobs.bg": {"203.0.113.5"}},
		txt: map[string][]string{
			"kmx.jobs.bg":                   {"v=spf1 ip4:198.51.100.7 -all"},
			"default._domainkey.kmx.jobs.bg": {"v=DKIM1; k=rsa; p=MIIBI..."},
		},
	}
	res, err := NewDomainCheckUsecase(loader, dns).Check(ownerCheckCtx(), "kmx.jobs.bg")
	if err != nil {
