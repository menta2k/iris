...
func TestDMARCRepro_HiddenCharBreaksCatcher(t *testing.T) {
	...
	const actualRcpt = "dmarc@kmx.verax.net"

	cases := []struct {
		name string
		addr string
	}{
		{"clean", "dmarc@kmx.verax.net"},
		{"hidden-char-localpart", "dmarc" + zwsp + "@kmx.verax.net"},
		{"trailing-nbsp", "dmarc@kmx.verax.net "},
		{"trailing-zwsp", "dmarc@kmx.verax.net" + zwsp},
	}
