// Package acmedns is a registry of DNS-01 challenge providers backed by
// go-acme/lego. Each registered provider exposes the metadata the admin UI
// needs to render a credentials form (required + optional fields), and a
// factory that turns a string-keyed config map into a challenge.Provider lego
// can drive.
//
// One factory function per provider lives in providers.go — operators don't
// need to grep across packages to find why a config key is named what it's
// named.
package acmedns
...
// ACMEChallenger is the interface lego expects a DNS-01 provider to satisfy. We
// re-export it so callers don't have to depend on lego's internal type for a
// one-method interface.
type ACMEChallenger = challenge.Provider

// ProviderFactory builds a challenge.Provider from a string-keyed configuration
// map. The map shape is provider-specific; ProviderInfo describes required and
// optional keys.
type ProviderFactory func(config map[string]string) (challenge.Provider, error)

// ProviderInfo is the metadata the admin UI surfaces when it renders a dynamic
// config form. Required + Optional are flat lists of camelCase keys; the UI
// shows them as text inputs and the factory parses them.
...
// RegisterProvider adds a provider to the global registry. Called from init()
// functions in this package. Returns an error if Name is empty, Factory is nil,
// or the provider is already registered.
...
// GetProvider creates a configured challenge.Provider by name, validating
// required fields up front (clearer error than letting lego complain about a
...
// IsRegistered reports whether a provider name is known.
...
// ListProviders returns the names of every registered provider in a stable
// (alphabetical) order — the UI uses this directly as the provider dropdown.
...
// GetProviderInfo returns a deep copy of the provider's metadata so callers
// can't mutate the registry by mutating the slices.
...
// AllProviderInfo returns deep copies of every provider's metadata in stable
// name order. Suitable for the UI's "registry" listing endpoint.
func AllProviderInfo() []*ProviderInfo {
	globalRegistry.mu.RLock()
	defer globalRegistry.mu.RUnlock()

	out := make([]*ProviderInfo, 0, len(globalRegistry.providers))
...
// getInt32 parses an int32 with a fallback, returning an error so the factory
// can surface "you set dnsTTL=banana" instead of silently using the default.
...
// getStringSlice splits a comma-separated config value into a trimmed slice.
...
// getJSONMap decodes a JSON-string config value into a string→string map. Used
// by hurricane (credentials per zone).
...
// sortStrings is a tiny dep-free insertion sort for the small provider lists.
