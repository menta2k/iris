...
scheduled_by_domain{domain="mail.example.com"} 5
scheduled_by_domain{domain="example.com"} 2
scheduled_count{queue="x"} 99
ready_count{queue="x"} 3
`)
		case "/api/admin/suspend/v1":
			_, _ = io.WriteString(w, `[{"id":"s1","domain":"mail.example.com","reason":"maintenance"}]`)
		default:
			http.NotFound(w, r)
		}
	}))
	...
	if byDom["mail.example.com"] != 5 || byDom["example.com"] != 2 {
		t.Fatalf("depths: %+v", byDom)
	}
	if !suspended["mail.example.com"] || suspended["example.com"] {
...
func TestSuspendAndBounceBuildRequests(t *testing.T) {
	...
	if _, err := k.SuspendQueue(context.Background(), "mail.example.com", "maint"); err != nil {
	...
	if body["domain"] != "mail.example.com" || body["reason"] != "maint" {
		t.Fatalf("suspend body: %s", gotBody)
	}

	if _, err := k.BounceQueue(context.Background(), "mail.example.com", ""); err != nil {
