...
// init registers every supported DNS-01 provider. Order doesn't matter
// — the registry sorts by name when listing. Each entry's
// RequiredFields / OptionalFields drives the UI form.
func init() {
...
// must is the init-time error escalation. A registration error is
// always a programming bug (duplicate name, nil factory) — letting it
// bubble would silently leave the registry incomplete.
func must(err error) {
	if err != nil {
...
// --- factories ------------------------------------------------------------
//
// Each factory pulls operator-supplied values from the config map (with
// the same camelCase keys the UI emits), translates them into a lego
// provider config, and constructs a challenge.Provider. The config
// fields below mirror lego's exported types; deviations are commented
// at the call site.
...
func factoryDigitalOcean(c map[string]string) (challenge.Provider, error) {
	...
	// digitalocean.Config.BaseURL is a string in lego — no URL parsing.
...
func factoryGCloud(c map[string]string) (challenge.Provider, error) {
	...
	// lego exposes three gcloud constructors. Service-account inputs use
	// the dedicated constructors (which load credentials from
	// file / inline key); when neither is set we use the env-driven
	// default which works for ADC / GKE workload identity.
