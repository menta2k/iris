...
// Whether the component has loaded its data at least once.
...
function transformData(data: OptionsItem[]): OptionsItem[] {
  ...
  return data.map((item, index) => {
    ...
    // Warn + synthesise a temp unique key when valueField is missing.
    if (value === undefined || value === null) {
      console.warn('node is missing the valueField', { item, valueField });
      // Generate a temp unique key to avoid collisions.
      const tempKey = `temp-key-${index}-${Date.now()}`;
      console.warn(`auto-generated temp key: ${tempKey}`);
    }
    return {
      ...objectOmit(item, [labelField, valueField, childrenField]),
      title: get(item, labelField),
      // Prefer value as the key; fall back to a temp key.
      key:
